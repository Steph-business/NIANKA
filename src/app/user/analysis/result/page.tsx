...
export default function AIDiagnosticTerminalPage() {
  ...
  const grade = analysisData.predicted_grade || "Grade A";
  const confidencePct = `${analysisData.confidence_pct || 95}%`;
  const korLbs = `${analysisData.metrics?.kor_lbs || 0} lbs`;
  const calibre = `${analysisData.metrics?.calibre_mm || 0}mm`;
  const defectRate = `${analysisData.metrics?.defect_rate_pct || 0}%`;
  const humidity = `${analysisData.metrics?.humidity_pct || 0}% HR`;
  const certification = analysisData.metrics?.certification || "Conforme Export EU";
  const certColor = analysisData.metrics?.certification_color || "#1a6b0a";
        ...
        <div style={{
          ...
          <div style={{ position: 'relative', width: '100%', height: '390px', borderRadius: '12px', overflow: 'hidden' }}>
            ...
            <div style={{
              ...
              <div><span style={{ color: '#94A3B8' }}>LATENCY:</span> <strong>14ms</strong></div>
              <div><span style={{ color: '#94A3B8' }}>MODEL:</span> model_anacarde.keras</div>
