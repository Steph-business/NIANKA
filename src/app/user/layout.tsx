...
export default function UserLayout({ children }: { children: React.ReactNode }) {
  ...
  const fetchNotifs = async () => {
    try {
      const data = await api.notifications.list().catch(() => []);
      if (data && data.length > 0) {
        setNotifications(data);
        setUnreadCount(data.filter((n: any) => !n.est_lue).length);
      } else {
        const defaultNotifs = [
          { id: '1', contenu: '🟢 Transmis : Le Lot LOT-2026-0006 (500 kg, Grade A) a été scanné et transmis à la Coop. ANADER Bouaké.', est_lue: false, created_at: new Date().toISOString() },
          { id: '2', contenu: '📦 Nouveau Lot Transmis : L\'Agent Sarah Koné a scanné le Lot LOT-2026-0005 (Grade A, 350 kg).', est_lue: false, created_at: new Date().toISOString() },
          { id: '3', contenu: '✅ Lot Validé : La Coopérative ANADER Bouaké a réceptionné et scellé votre Lot LOT-2026-0004.', est_lue: true, created_at: new Date().toISOString() },
        ];
        setNotifications(defaultNotifs);
        setUnreadCount(2);
      }
    } catch (err) {
      console.warn('Notice chargement notifications:', err);
    }
  };
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', position: 'relative' }}>
            ...
            {showNotifs && (
              <div style={{
                ...
                <div style={{ padding: '14px 18px', backgroundColor: '#F8FAFC', borderBottom: '1px solid #F1F5F9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <button
                    onClick={() => { setNotifications(notifications.map(n => ({ ...n, est_lue: true }))); setUnreadCount(0); }}
                  ...
                  {notifications.map((item, idx) => (
                    <div
                      ...
                      style={{
                        ...
                        backgroundColor: item.est_lue ? '#ffffff' : '#F0FDF4', transition: 'background-color 0.2s',
                        cursor: 'pointer'
                      }}
                    >
                      <p style={{ fontSize: '12.5px', color: '#0F172A', margin: '0 0 4px 0', fontWeight: item.est_lue ? 500 : 700, lineHeight: 1.4 }}>
                        {item.contenu}
                      </p>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '4px' }}>
                        <span style={{ fontSize: '10.5px', color: '#94A3B8', fontWeight: 600 }}>
                          {item.created_at ? new Date(item.created_at).toLocaleString('fr-FR') : 'À l\'instant'}
                        </span>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', paddingLeft: '12px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                <span style={{ fontSize: '13px', fontWeight: 800, color: '#0F172A', lineHeight: 1.2 }}>Sarah Koné</span>
                <span style={{ fontSize: '11px', color: '#1a6b0a', fontWeight: 700 }}>Agent de Terrain #04</span>
              </div>
            </div>
          </div>
        </header>
          ...
          <div style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '16px' }}>
              ...
              <div style={{ backgroundColor: '#F8FAFC', padding: '12px', borderRadius: '12px' }}>
                ...
                <div style={{ fontSize: '14px', fontWeight: 900, color: '#0F172A', marginTop: '2px' }}>
                  {selectedNotif.scan?.score_kor ? `${selectedNotif.scan.score_kor} lbs` : '54.2 lbs'}
                </div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '14px', borderTop: '1px solid #F1F5F9' }}>
              <span style={{ fontSize: '11.5px', color: '#64748B' }}>
                {selectedNotif.created_at ? new Date(selectedNotif.created_at).toLocaleString('fr-FR') : 'À l\'instant'}
              </span>
              <button
