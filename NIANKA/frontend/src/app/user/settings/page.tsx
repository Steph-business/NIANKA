...
import { api, getCurrentUserProfile, clearAuthSession, UserProfile } from '@/lib/api';
import { initialesDe } from '@/lib/useUserProfile';
import { useRouter } from 'next/navigation';

const LABEL_ROLE: Record<string, string> = {
  agent: 'Agent de Terrain NIANKA',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '20px', marginBottom: '28px', paddingBottom: '24px', borderBottom: '1px solid #F8FAFC' }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: '13px', color: '#1a6b0a', margin: '0 0 4px 0', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                      Rôle : {LABEL_ROLE[profile?.role ?? ''] ?? profile?.role ?? '—'}
                    </p>
                    <span style={{ fontSize: '12px', color: '#64748B', fontWeight: 500 }}>Statut : Compte Vérifié</span>
                  </div>
                </div>

                {saveError && (
                  <div style={{ backgroundColor: '#FEF2F2', border: '1.5px solid #FCA5A5', color: '#991B1B', padding: '12px 16px', borderRadius: '10px', fontSize: '13px', fontWeight: 700, marginBottom: '18px' }}>
                    {saveError}
                  </div>
                )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div style={{ backgroundColor: '#ffffff', borderRadius: '16px', padding: '20px', boxShadow: '0 4px 20px rgba(0,0,0,0.03)', borderLeft: '4px solid #10B981', borderTop: '1px solid #F1F5F9', borderRight: '1px solid #F1F5F9', borderBottom: '1px solid #F1F5F9' }}>
                ...
                <p style={{ fontSize: '12px', color: '#64748B', margin: 0, lineHeight: 1.5, fontWeight: 500 }}>
                  Connexion sécurisée par numéro de téléphone, avec session active protégée.
                </p>
              </div>
