...
export default function InstitutionDashboard() {
  ...
  const tonnageExpedie = stats?.tonnage_expedie;
  const tauxExpedition = tonnageExpedie != null && stats?.tonnage_total_collecte
    ? `${Math.round((tonnageExpedie / stats.tonnage_total_collecte) * 100)}%`
    : '—';

  const regionalData = [
    { region: 'Agrégat National (toutes zones actives)', volume: totalVolume, kor: korAverage, gradeA: premiumPct, status: `${stats?.lots_scelles ?? 0} lots scellés` },
  ];
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px' }}>
        <div style={{ backgroundColor: '#ffffff', borderRadius: '16px', padding: '24px', boxShadow: '0 4px 20px rgba(0,0,0,0.03)' }}>
          ...
          <span style={{ fontSize: '11px', fontWeight: 800, color: '#10B981', backgroundColor: '#ECFDF5', padding: '3px 8px', borderRadius: '10px', marginTop: '8px', display: 'inline-block' }}>
            {stats?.total_bordereaux ?? 0} lots certifiés au total
          </span>
        </div>
        ...
        <div style={{ backgroundColor: '#1a6b0a', color: '#ffffff', borderRadius: '16px', padding: '24px', boxShadow: '0 6px 24px rgba(26, 107, 10, 0.25)' }}>
          <div style={{ fontSize: '12px', fontWeight: 800, color: 'rgba(255,255,255,0.85)' }}>TAUX D&apos;EXPÉDITION (VS COLLECTE)</div>
          <div style={{ fontSize: '34px', fontWeight: 900, marginTop: '8px', lineHeight: 1 }}>{tauxExpedition}</div>
          <p style={{ fontSize: '11.5px', color: 'rgba(255,255,255,0.9)', margin: '8px 0 0 0', fontWeight: 500 }}>
            {tonnageExpedie != null ? `${tonnageExpedie} T expédiées sur ${stats?.tonnage_total_collecte ?? 0} T collectées.` : 'Donnée de tonnage expédié non encore disponible.'}
          </p>
        </div>
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
          <thead>
            <tr style={{ borderBottom: '1.5px solid #F1F5F9', backgroundColor: '#F8FAFC' }}>
              ...
              <th style={{ padding: '14px 16px', fontSize: '11.5px', fontWeight: 800, color: '#64748B', textAlign: 'right' }}>LOTS SCELLÉS</th>
            </tr>
          </thead>
          <tbody>
